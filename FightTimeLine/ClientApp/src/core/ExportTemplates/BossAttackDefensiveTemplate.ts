    ...
    build(data: ExportData): IExportResultSet {
        ...
        const rows = data.data.boss.attacks
            ...
            .map(it => [
                ...
                data.data.bossTargets.filter(bt => this.isOffsetInRange(it.offset, bt.start, bt.end)).map(bt => jobs.find(j => j.id === bt.target)).map(it => <IExportResultItem>{ text: it.name, icon: it.icon }),
                ...jobs.map(t => {
                    return data.data.abilities
                        .filter(a =>
                            used.indexOf(a) === -1 &&
                            a.job === t.id &&
                            (((a.type & 1) === 1) || ((a.type & 256) === 256)) &&
