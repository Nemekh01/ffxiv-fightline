...
export class Parser {
  public players: FFLogs.IJobInfo[];
  ...
  public createFilter(jobRegistry: Jobregistryserviceinterface.IJobRegistryService): string {

    const enemyIds = this.rawFight.enemies.map(e => e.guid).join();

    const js = jobRegistry.getJobs().filter(j => this.players.some((j1 => j1.job === j.name) as any));
    ...
    const partyIds = this.players.map(j => j.guid).join();
